angular.module('ticketApp').directive('viewTicket',["$http", "ApiService", function($http, ApiService){
	return {
		replace : true,
		restrict : 'EA',
		transclude: false,
...
/*&& $scope.selectedAgent.length >= 1 
&& $scope.selectedAgent.indexOf(ticket.ticket_assigned_to) >= 0*/

      if($scope.selectedStatus.length >= 1 ){
        $scope.filteredTickets = $scope.tickets.filter(function(ticket){
          return ($scope.selectedStatus.indexOf(ticket.ticket_status) >= 0 )
        })
      }else{
    ...
    $scope.existAgent = function(item){
      return $scope.selectedAgent.indexOf(item.userID) > -1;
    }

    $scope.toggleSelectionAgent = function(item){
      var idx = $scope.selectedAgent.indexOf(item.userID);
      if (idx > -1) {
      ...
      if($scope.selectedAgent.length >= 1){
        $scope.filteredTickets = $scope.tickets.filter(function(ticket){
          return ($scope.selectedAgent.indexOf(ticket.ticket_assigned_to) >= 0)
        })
      }else{
        $scope.filteredTickets = $scope.tickets;
      }
    ...
    $scope.checkAllAgent = function(){
      if ($scope.selectAllAgent) {
        angular.forEach($scope.agents, function(item){
          idx = $scope.selectedAgent.indexOf(item.userID);
          if (idx >= 0) {
            return true;
          }
          else{
...
$scope.sort_with = function(base){
  ...
  $scope.reverse = !$scope.reverse;
		...
		templateUrl: 'pages/view-all-ticket.php'
	};
}]);